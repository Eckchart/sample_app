...
class PasswordResetForm < PasswordResets
  def setup
    ...
    # This is the `@user` variable in the PasswordResets#create action.
...
class PasswordFormTest < PasswordResetForm
    ...
    # Reset link has wrong email.
    test "reset with wrong email" do
      get edit_password_reset_path(@reset_user.reset_token, email: "")
      assert_redirected_to root_url
    end

    # Reset link is for an unactivated user.
    test "reset with unactivated user" do
    ...
    # Reset link has the wrong token.
...
class ExpiredToken < PasswordResets
  def setup
    ...
    # Attempt to update the user's password (via the
